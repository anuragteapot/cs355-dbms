-- 1
select * from Faculty_details fd LEFT JOIN Supervisor s ON fd.faculty_id = s.faculty_id WHERE s.student_id is NULL;

-- 2
select sd.* from Student_details sd INNER JOIN Author_details ad ON sd.student_id = ad.author_id AND ad.author_type = 'student' WHERE sd.student_institute = 'IIT Patna';
...
SELECT pd.paper_title FROM Paper_details pd
  ...
  join Paper_author pa on fd.faculty_id = pa.author_id group by
  fd.faculty_id;

  -- 7
  select author_id from Paper_author group by author_id order by count(*)
  desc limit 1;

  -- 8
  select sd.* from Student_details sd left join Paper_author pa on
  pa.author_id=sd.student_id where pa.author_id is NULL;

  -- 9
  select sd.* from Student_details sd inner join Supervisor s on
  s.student_id = sd.student_id group by sd.student_id having
  count(sd.student_id) = 2;
